...
public class InputManager : MonoBehaviour {
    ...
    public GameObject objectToControl {get; set;}
    ...
    private void Start()
    {
        ...
        playerRigidbody = objectToControl.GetComponent<Rigidbody>();
        controller = objectToControl.GetComponent<CharacterController>();
        canWalk = true;
    }
    private void Update()
    {
    ...
    void PlayerMovement()
    {
        ...
        if (canWalk)
        {
            ...
            if (grounded)
            {
                ...
                //transform direction
                moveDirection = objectToControl.transform.TransformDirection(moveDirection);
        ...
        if (Input.GetMouseButton(1))
        {
            objectToControl.transform.rotation = Quaternion.Euler(0, Camera.main.transform.eulerAngles.y, 0);
        }
        else
        {
            objectToControl.transform.Rotate(0, Input.GetAxis("Horizontal") * turnSpeed * Time.deltaTime, 0);
        }
    ...
    public void switchCharacter(GameObject newchar)
    {
        objectToControl = newchar;
        playerRigidbody = objectToControl.GetComponent<Rigidbody>();
        controller = objectToControl.GetComponent<CharacterController>();
    }
}
